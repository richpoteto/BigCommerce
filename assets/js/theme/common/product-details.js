...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        this.defaultPrice = document.querySelector(".price.price--withoutTax.price--main")?.innerHTML ?? document.querySelector(".price.price--withTax").innerHTML
        this.priceParsed = parseFloat(this.defaultPrice.substring(1))
        ...
        const placeSwatchLabelImage = (_, label) => {
            ...
            if (emptySpaceToScreenRightBorder < (optionImageWidth + extendedOptionImageOffsetLeft)) {
				...
				$('.custom-product-list .productOptions-list-item').on('click', event => {
				let $Item = $(event.target);
				let Src = $Item.closest('.productOptions-list-item').attr('data-img');
				if (Src != undefined) {
					let ProdArr = Src.split("products/");
					let ProdIdArr = ProdArr[1].split("/");
					document.location.href = '/products.php?productId=' + ProdIdArr[0];
					console.log('sadsad=' + ProdIdArr[0]);
				}
				...
				if ($('.volume-multisku-custom-wrapper').length) {	
		
	
		//let InitialPrice = $('.price').html().replace('$','').replace(',','').replace("CDN",'');
		let InitialPrice = $('.productView-price meta[itemprop="price"]').attr('content')
		let InitialPricePerItem = Math.round(InitialPrice / 1 * 1000 ) / 1000;
		
		$('.volume-discount').each(function() {
			let DiscountType = $(this).data("type");
			let Discount = 0;
			let discountAmount = 0;
			
			//console.log('test=' + $('.productView-price meta[itemprop="price"]').attr('content'))
			
			if (DiscountType == 'percent') {
				discountAmount = $(this).html().replace('%','').replace(',','').replace("CDN",'').replace("$",'');
				//Discount = InitialPricePerItem / 100 * (100-discountAmount);
				//Discount = Math.round(Discount * 1000) / 1000;
				Discount = InitialPrice / 100 * (100-discountAmount);
				...
				discountAmount = $(this).data('discount').replace('$','').replace(',','').replace("CDN",'').replace("%",'');
				
				//console.log('discount=' + discountAmount);
				
				
				let ItemInitialPrice = InitialPricePerItem - discountAmount;	
				
				//console.log('ItemInitialPrice=' + ItemInitialPrice);				
					
				  Discount = Math.round(ItemInitialPrice * 100 ) / 100;
				  Discount = Discount.toFixed(2);
				$(this).html('$' + Discount);	
				} else {
					//InitialPrice = $(this).html().replace('%','').replace(',','').replace("CDN",'');
					//InitialPricePerItem = Math.round(InitialPrice / QuantityPerCase * 1000 ) / 1000;
					//Discount = Math.round(InitialPrice * 1000 ) / 1000;
					//$(this).html('$' + Discount);	
				}
			}
		     ...
		     $('.product-duscount-box').on('click', event => {
				let $discountId = $(event.target);
				let $tieredPrice = $(event.target);
				
				if ($discountId.hasClass('product-duscount-box') == false) {
					$tieredPrice = $discountId.closest('.product-duscount-box');
				}
				
				let newQnt = $tieredPrice.data('min');
				let newPrice = $tieredPrice.find('.product-duscount-box--body-unit').html();
				
				console.log('test=' + $tieredPrice.data('min'));
				
				$('.form-input.form-input--incrementTotal').val(newQnt);
				$('.price-section .price--main').html(newPrice);
				
				
				if ($tieredPrice.hasClass('selected')) {					
					$tieredPrice.removeClass('selected');
				} else {
					$('.product-duscount-box').removeClass('selected');
					$tieredPrice.addClass('selected');
				}
			});
    ...
    listenQuantityChange() {
		...
		function CheckTiers(qty) {
				$('.product-duscount-box').each(function(){
					let TierElem = $(this);
					let tierMin = TierElem.data('min');
					let tierMax = TierElem.data('max');
					
					if (TierElem.hasClass('smallest') && qty < tierMin) {
						$('.product-duscount-box').removeClass('selected');
					} else {
					
					if (qty >= tierMin &&  qty <= tierMax ) {
						if ($('.product-duscount-box[data-min="' + tierMin + '"]').hasClass('selected') == false) {
							$('.product-duscount-box').removeClass('selected');
							$('.product-duscount-box[data-min="' + tierMin + '"]').addClass('selected');
						}
						}
					}
				});
        ...
        this.$scope.on('click', '[data-quantity-change] button', event => {
            ...
            const bulkPricingEl = document.querySelector("#bulk-pricing-data")
            if (bulkPricingEl) {
                const data = JSON.parse(bulkPricingEl.innerHTML)

                const res = data.find(item => item.max >= qty && item.min <= qty)
                const max = data.filter(item => item.min <= qty)

                const el = document.querySelector(".price.price--withoutTax.price--main") ?? document.querySelector(".price.price--withTax")
                if (data[0].type === "price") {
                    if (res) {
                        console.log(res)
                        el.innerHTML = "$" + (this.priceParsed - res.discount.value)
                    }
                    
                    if (!res && max.length > 0){
                        el.innerHTML = "$" + (this.priceParsed - max[max.length - 1].discount.value)
                    }
                    if (!res && max.length === 0) {
                        el.innerHTML = this.defaultPrice
                    }
                } else {
                    if (res) {
                        el.innerHTML = res.type === "percent" ? "$" + (this.priceParsed - (this.priceParsed * (res.discount.value / 100))).toFixed(2) : res.discount.formatted
                    }
                    
                    if (!res && max.length > 0){
                        el.innerHTML = max[max.length -1].type === "percent" ? "$" + (this.priceParsed - (this.priceParsed * (max[max.length - 1].discount.value / 100))).toFixed(2) : max[max.length - 1].discount.formatted
                    }
                    if (!res && max.length === 0) {
                        el.innerHTML = this.defaultPrice
                    }
